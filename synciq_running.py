...
running_job = job_list['jobs']
for item in running_job:
    if item['error']:
        success = False
    else:
        success = True
...
"""
filename = "repo/isilon_sycnip_running_" + cluster + "_" + collection_date
with open(filename, 'a') as output:
    output.write('collection_date, cluster\n')
    output.write(f'{collection_date}, {cluster}\n')
    output.write(f'\n')
    output.write(f'{running_job}\n')
#print(filename)
"""

es = connect_elasticsearch()

resp = helpers.bulk(es, running_job, index='isilon_job_list')
