...
response = requests.request("GET", f'https://{url}:9669/v1/vms', headers=headers, data=payload, verify=False)
...
for info in data:
    x = {'vpg_name': data[count]['VpgName'], 'vm_name': data[count]['VmName'], 'timestamp': timestamp}
    count += 1
    data_list.append(x)

es = connect_elasticsearch()
resp = helpers.bulk(es, data_list, index='zerto_vms')
